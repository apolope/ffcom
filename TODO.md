...
- [x] Ligar `automaticSilentRenew: true` — implementado via refresh token (`offline_access`), não iframe; exige `ak apply_blueprint` do provider `ffcom` atualizado (`abs-3d-printer`) na instância central de Authentik antes de valer em produção. Ver `docs/architecture.md`, "Decisão: renovação silenciosa de sessão no client"
