...
- [ ] Repassar o coturn aos clientes pelo LiveKit (`rtc.turn_servers` na config de `deploy/channel/docker-compose.yml` e do compose de referência, com a credencial derivada de `TURN_STATIC_AUTH_SECRET`) — hoje o coturn roda mas nenhum client o usa, então quem tiver UDP 50000-50100 e TCP 7881 bloqueados fica sem voz. Conferir antes se o `TCP 7881` também é encaminhado por `VMSUBS24OCI0102` (só o UDP foi testado)
...
- [ ] Botão de deslogar — `signOut` já existe em `client/src/auth/AuthProvider.tsx` (`userManager.signoutRedirect()`, `post_logout_redirect_uri` = origem do app), mas nenhum componente o chama; falta o botão (provavelmente no rodapé do `ServerRail`, ao lado do botão de avatar). **Junto com ele, resolver o vazamento entre contas no mesmo navegador:** o par de chaves de E2E (`ffcom.e2e.keypair.v1`, `crypto/e2e.ts`) e a flag de publicação (`ffcom.e2e.publishedKey.v1`, `hooks/useE2EKeys.ts`) não são por conta, então quem logar depois de outra pessoa herda a chave privada dela e, como a flag já bate, nunca publica a própria chave em `server-central` (DM para essa conta falha). Os cursores de não lida (`lib/unread.ts`) têm o mesmo problema, com impacto menor. Opções: chavear o `localStorage` pelo `sub` do OIDC ou limpar tudo no logout (limpar perde as DMs antigas deste dispositivo, ver "Decisão: criptografia ponta-a-ponta em DMs")
