...
**Contexto:** o único jeito de criar amizade era o convite por código (decisão acima), que exige combinar por fora. Quem já divide um server-channel com alguém pediu para adicionar direto pela lista de membros, com clique direito, e que a outra pessoa aprove ou recuse.

**Alternativas consideradas:** gerar um convite por código escondido e entregá-lo ao outro lado; ativar o fluxo `Request`/`SetStatus` que o `FriendshipStore` já tinha desde a migration `0001_init` (status `pending`), com endpoints e frames próprios.

**Decisão:** pedido explícito com status `pending`, sem migration nova. `POST /api/friends/requests {accountId}` cria o pedido (`FriendshipStore.Request`, que recusa se já houver qualquer relação entre as duas contas em qualquer direção); se o outro lado já tinha pedido, o pedido dele é aceito na hora. `POST /api/friends/requests/{id}/accept` só funciona para o destinatário (`FriendshipStore.Accept`), e `DELETE /api/friends/requests/{id}` recusa ou cancela apagando a linha (`DeletePending`). Cada mudança avisa os dois lados pelo WebSocket de presença (`friend.request`, `friend.request.removed`, `friend.accepted`, ver `docs/protocol.md`), e o client mostra o pedido recebido como aviso no canto e na seção "Pedidos recebidos" de `FriendsView`, com o botão de Amigos do rail marcado enquanto houver pedido.

**Razão:** o `accountId` do outro lado só chega ao client via `POST /api/accounts/lookup` a partir do `oidcSubject` de um membro, ou seja, de quem divide um servidor com a conta; isso preserva a filosofia de "sem diretório pesquisável" da decisão do convite. O fluxo `pending` já estava modelado e não pedia infraestrutura nova, e um convite escondido fingiria consentimento de quem nunca clicou em nada.

**Consequências:** amizade `pending` não libera DM nem presença (`AreFriends` e `AcceptedFriendIDs` continuam só com `accepted`), o que responde a pergunta deixada em "Revisitar quando" da decisão de DMs. Resgatar um convite entre duas contas com pedido pendente promove o pedido a `accepted` em vez de dar 409. Recusar não deixa rastro, então quem foi recusado pode pedir de novo; o rate limit por usuário é o único freio hoje.

**Revisitar quando:** aparecer pedido repetido indesejado na prática (guardar a recusa, ou usar o status `blocked` que o schema já aceita), ou quando o nome de exibição passar a existir para toda conta; hoje, sem perfil em `server-central`, o pedido mostra o começo do `accountId`, mesmo limite da lista de amigos.
...
**Cliente:** `lib/serverChannelApi.ts` ganhou `sendUpdateMessage`/`sendDeleteMessage` e os dois frames novos em `ChannelSocketFrame`; `hooks/useChannelChat.ts` aplica `message.updated` (substitui a mensagem na lista pelo id) e `message.deleted` (remove pelo id) e expõe `editMessage`/`deleteMessage`. `components/TextChannelView.tsx` mostra "editar"/"apagar" só na própria mensagem (`authorMemberId === selfMemberId`), ao passar o mouse — apagar é direto, editar troca o conteúdo por um form inline reaproveitando o mesmo frame de saída; **Atualização (2026-09-22):** quem tem `Administrator` na permissão base (ou é dono) vê "apagar" também na mensagem dos outros (`canModerateMessages`, calculado em `App.tsx` e passado por `MainPanel`). Diferente da própria mensagem, pede um segundo clique ("confirmar apagar", sem `confirm()` do navegador), porque quem modera não tem como recuperar o que apagou por engano. O client olha só a permissão base: um `Administrator` concedido por `Allow` de overwrite num canal só (o servidor aceita, porque `Effective` soma o `allow`) não faz o botão aparecer; nesse caso raro o servidor aceitaria o frame, mas a UI não oferece. Editar mensagem alheia continua fora (servidor também recusa).
...
## Decisão: rate limit por usuário e dispositivo

**Contexto:** com a chave só por `sub` (decisão anterior), a mesma conta aberta no celular, no PC e no notebook dividia um balde por servidor, e a rajada de abertura de um aparelho derrubava os outros em `429`. O uso normal de uma pessoa com vários dispositivos é esperado e não deveria ser tratado como abuso.

**Alternativas consideradas:** (1) id de dispositivo gerado pelo client (UUID em `localStorage`) mandado em header, o que exigiria mexer em toda chamada de `serverCentralApi.ts`/`serverChannelApi.ts`, liberar o header no CORS dos dois servidores e passar por query string no WebSocket, e principalmente deixaria qualquer client inventar um id novo por requisição para multiplicar o próprio orçamento; (2) `sub` + IP, que junta celular, PC e notebook no mesmo Wi-Fi de casa, justamente o caso que motivou a mudança; (3) claim `sid` do access token (adotada).

**Decisão:** `auth.Claims` ganhou `SessionID` (`sid`). O Authentik (2026.8.3 na instância central) emite o access token a partir do ID token e inclui o `sid`, que é o SHA-256 da chave da sessão de login do navegador/app. `auth.IdentifyRequest` devolve `sub` e `sid` e anexa os dois ao contexto; `withRateLimit` usa a chave `sub:<sub>|sid:<sid>`, ou `sub:<sub>` quando o token não traz `sid`. O limiter de frames do WebSocket de canal passou de `member.ID` para `member.ID|sid:<sid>` pelo mesmo motivo, lendo o `sid` via `auth.SessionFromContext` (o handshake passou por `withRateLimit`). O client não mudou.

**Razão:** o `sid` já vem assinado pelo Authentik, então a separação por dispositivo não abre brecha para o client forjar baldes; para ganhar outro balde é preciso fazer um login novo de verdade. Cada dispositivo loga por conta própria e recebe sessão própria, que é exatamente a granularidade pedida. Abas do mesmo navegador compartilham a sessão e continuam dividindo o balde, o que é aceitável (é o mesmo aparelho). Sem `sid`, o comportamento é o da decisão anterior, então não há regressão.

**Conferido num token real (2026-09-26, conta apolonio.serafim, client em dev):** o access token do login traz `sid` (64 caracteres, hash SHA-256). Depois de uma renovação pelo grant `refresh_token` (`userManager.signinSilent()`, só chamadas a `/application/o/token/`, sem `/authorize`; `iat` e token diferentes), o `sid` continua presente e com o mesmo valor. Um login OIDC novo no dia seguinte, no mesmo navegador, também manteve o mesmo `sid`: ele identifica a sessão do navegador no Authentik, não cada login, o que dá um balde por navegador. Uma segunda sessão da mesma conta (janela anônima) recebeu `sid` diferente, confirmando baldes separados por navegador/dispositivo. Não testado: renovação depois que a sessão do Authentik expira com o refresh token ainda válido; se o `sid` sumir nesse caso, o dispositivo cai no balde só do usuário (fallback já tratado), sem erro.

**Verificado (2026-09-25):** `go build`, `go vet` e `go test` em `internal/httpapi` dos dois módulos, com teste novo para mesma conta em dispositivos diferentes e mesmo `sid` em contas diferentes.
...
## Decisão: TURN repassado pelo LiveKit — `rtc.turn_servers` com `secret`, credencial temporária

**Contexto:** os dois composes (`server-channel/docker-compose.yml` de referência e `deploy/channel/docker-compose.yml` da instância oficial) subiam o coturn, mas a config do LiveKit não tinha `rtc.turn_servers`, então nenhum client recebia o TURN no ICE config. Achado na "Correção: IP anunciado pelo LiveKit".

**Alternativas consideradas:**
- **Credencial fixa (`username`/`credential`) derivada uma vez de `TURN_STATIC_AUTH_SECRET`:** no esquema do `--use-auth-secret` o usuário carrega a data de expiração, então ou a credencial vence e a voz para, ou é emitida com expiração distante e vira senha permanente entregue a todo client. Descartada.
- **TURN embutido do LiveKit (`turn:` na config):** tiraria um container, mas só vale a pena com TLS (certificado no próprio LiveKit), e o coturn já está no ar, com portas encaminhadas por `VMSUBS24OCI0102`. Adiado junto com o TURN/TLS abaixo.
- **`secret` em `rtc.turn_servers`:** escolhida. O LiveKit gera, a cada join, usuário `<expiração>:<id>` e senha `base64(HMAC-SHA1(secret, usuário))`, que é exatamente o que o coturn valida com `--use-auth-secret`; TTL padrão de 4h.

**Decisão:** duas entradas (UDP e TCP) com `host: ${TURN_EXTERNAL_IP}` e `secret: ${TURN_STATIC_AUTH_SECRET}`. Porta `${TURN_LISTEN_PORT}` (33478) no compose de implantação, que é a porta do host, encaminhada igual por `VMSUBS24OCI0102`; `3478` fixo no de referência, onde a porta publicada já é essa. O segredo vai entre aspas no YAML para não quebrar com `+`, `/`, `=` ou `&`.

**Verificado (2026-09-22):** `livekit/livekit-server:latest` (1.13.7) sobe com a config renderizada por `docker compose config`; um join de teste (JWT gerado localmente, `SignalResponse` decodificado com `@livekit/protocol`) devolve `turn:<ip>:33478?transport=udp` e `?transport=tcp` com credencial que confere com o HMAC-SHA1 do segredo via `openssl`. Daqui, TCP em `137.131.249.145:7881` e `:33478` aceita conexão (saindo da rede do próprio site, pode ter passado por hairpin). Não verificado: alocação TURN de verdade por um client de fora, que fica para o teste de voz com 2 pessoas em redes diferentes.

**Limitação, para não vender além do que entrega:** TURN em UDP/TCP 33478 cobre quase os mesmos casos que o fallback ICE-TCP `7881` do LiveKit já cobria. As redes que de fato derrubam voz (corporativa, hotel, algumas universidades) costumam liberar só 443, e para essas o que resolve é TURN/TLS na 443, que exige certificado e, nesta infra, roteamento por SNI em `VMSUBS24OCI0102` (o NPM já ocupa a 443 lá). Registrado no TODO.
...
**Bug corrigido em 2026-09-23 (`client-v0.6.2`):** na `v0.6.1` o botão aparecia mas o clique não fazia nada. O `updateServiceWorker(true)` do `vite-plugin-pwa` só manda `SKIP_WAITING`; o reload depende do evento `controlling` do `workbox-window` com `isUpdate`, que não vem numa página aberta sem service worker controlando (Ctrl+F5, ou a primeira visita). A versão nova ativava em silêncio e a página ficava na antiga. Reproduzido no Chrome com `vite preview` numa origem nova (primeira visita = página sem controlador): depois de `registration.update()`, nenhum service worker esperando e a página ainda com o JavaScript antigo. Corrigido em `useAppUpdate`: o próprio `applyUpdate` recarrega quando o worker que estava esperando chega a `activated` (com prazo de 3s de segurança), e recarrega direto se não há nada esperando. Mesmo cenário depois da correção: a página recarregou sozinha na versão nova, já controlada. **Lição:** o teste anterior passou porque a página tinha sido recarregada normalmente antes, e portanto era controlada; testar também a primeira visita.
...
5. **Som ao apertar e soltar (revisto em 2026-09-23):** a primeira versão não tocava nada, porque o bipe tocaria a cada fala e o de abrir toca com o microfone já aberto, o que pode vazar pelo ar com alto-falante. No primeiro teste real (`client-v0.8.0`) o dono sentiu falta do som nos dois sentidos, e o retorno visual não bastou para quem aperta a tecla sem olhar a tela (o Discord também toca um som ao apertar e ao soltar). Agora `playPushToTalkSound` (`lib/micToggleSound.ts`) toca uma nota só, mais curta e baixa que a do mute (50 ms, pico 0,08): 660 Hz ao apertar e 440 Hz ao soltar. Toca no aperto e no soltar (este já com os 200 ms de atraso), sem esperar a troca do microfone resolver, porque aqui o som é o retorno da tecla e abrir um microfone já publicado é só desmutar. Trocar de modo conectado não toca. O liga/desliga é o mesmo `micToggleSound`, e o checkbox agora aparece nos dois modos ("Som ao mutar" ou "Som ao apertar e soltar"). O risco de o bipe de abrir vazar pelo alto-falante continua, e o cancelamento de eco deve cuidar da maior parte, como no som de desmutar.
...
**Alternativas consideradas:**
- **Só a supressão do navegador (o que já existe):** custo zero, nada a baixar, igual no web e no Electron (ambos Chromium). Pelo que se sabe do supressor do WebRTC, ele lida bem com ruído constante (ventilador, chiado) e mal com ruído de impulso (teclado, cachorro, louça), mas a qualidade varia por navegador e sistema e não medi nada aqui. Continua como padrão.
- **RNNoise (Xiph, BSD-3-Clause) em WASM num `AudioWorklet`:** rede pequena feita para voz em tempo real, usada em vários projetos WebRTC. O pacote `@sapphi-red/web-noise-suppressor` (MIT; versão 0.4.1 publicada em 2026-09-19) traz o nó pronto: `rnnoise_simd.wasm` com ~157 KB mais um worklet de ~64 KB, carregados só quando a opção está ligada. Processa quadros de 480 amostras a 48 kHz. Pelo buffer do worklet, a latência extra parece ficar na ordem de 10 a 20 ms, a medir. Escolhida como opção "reforçada".
- **GTCRN (MIT, mesmo pacote, `gtcrn.wasm` de ~197 KB):** modelo mais novo e aceita contexto de 16 kHz ou 48 kHz. Não achei comparação confiável com o RNNoise, e trocar um pelo outro no mesmo pacote é trocar o nó. Fica como candidata no teste de escuta, sem decisão.
...
3. **Pontos de implementação já identificados:**
   - **`AudioContext` próprio a 48 kHz dentro do processador.** O `setProcessor` entrega o `AudioContext` da sala, que o LiveKit cria com `new AudioContext({ latencyHint: 'interactive' })` na taxa do dispositivo (pode ser 44,1 kHz), e o RNNoise espera 48 kHz.
   - **Com a reforçada ligada, capturar com `noiseSuppression: false` e `voiceIsolation: false`**, mantendo `echoCancellation` e `autoGainControl`, para não empilhar dois supressores. Trocar a opção conectado exige `restartTrack` com as novas constraints ou vale na próxima entrada; decidir na implementação.
   - **WASM e worklet via `?url` do Vite, carregados só ao ligar.** Conferir que o precache do PWA pega o `.wasm` (o padrão do Workbox deve incluir, mas o build dirá) e que o `app://` do Electron serve os dois (o pacote baixa o WASM com `fetch().arrayBuffer()`, o que não depende do MIME).
