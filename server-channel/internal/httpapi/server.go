...
//
// restRateLimitRPM/restRateLimitBurst configuram o limiter por usuário (IP
// só sem token válido) sobre toda a API REST (incluindo o handshake de WS); wsRateLimitRPM/wsRateLimitBurst
...
func NewRouter(verifier *auth.Verifier, db *store.Store, attachmentFiles *storage.FileStore, attachmentMaxBytes int64, allowedOrigins []string, liveKitAPIKey, liveKitAPISecret, liveKitPublicURL, liveKitAPIURL, version string, requireTLS bool, restRateLimitRPM, restRateLimitBurst, wsRateLimitRPM, wsRateLimitBurst int) http.Handler {
	...
	mux.Handle("PUT /api/categories/order", protected(handleReorderCategories(db.Categories, db.Roles)))
	...
	mux.Handle("PUT /api/channels/order", protected(handleReorderChannels(db.Channels, db.Roles)))
